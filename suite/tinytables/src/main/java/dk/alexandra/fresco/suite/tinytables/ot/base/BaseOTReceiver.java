...
public class BaseOTReceiver implements dk.alexandra.fresco.suite.tinytables.ot.OTReceiver {
  ...
  private OTSemiHonestDDHBatchOnByteArrayReceiver getInstance(
      SecureRandom random) {
    if (receiver == null) {
      receiver = ExceptionConverter.safe(
          () -> new OTSemiHonestDDHBatchOnByteArrayReceiver(
              new edu.biu.scapi.primitives.dlog.bc.BcDlogECF2m(), KdfFactory
                  .getInstance().getObject("HKDF(HMac(SHA-256))"), random),
          "Could not initialize receiver functionality of SCAPI OT");
    }
    return receiver;
  }
  ...
  public List<BitSet> receive(List<OTSigma> sigmas, int expectedLength) {
    ...
    NetworkWrapper wrapper = new NetworkWrapper(network, myId);
    OTBatchOnByteArrayROutput output = ExceptionConverter.safe(
        () -> (OTBatchOnByteArrayROutput) receiver.transfer(wrapper, input),
        "Could not receive message via SCAPI OT.");

    List<BitSet> results = new ArrayList<BitSet>();
    for (int i = 0; i < sigmas.size(); i++) {
      results.add(dk.alexandra.fresco.suite.tinytables.ot.Encoding.decodeBitSet(
          output.getXSigmaArr().get(i)));
    }
    return results;
  }
